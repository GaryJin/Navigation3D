 ...
 */
TEST(birrtStarTreeTest, TestWithObstacle) {
    ...
    BiRRTStar<Vector2f> *tree = new BiRRTStar<Vector2f>(_stateSpace);
...
TEST(birrtStarTreeTest, plane3d_withob) {
    ...
    //	give it plenty of iterations so it's not likely to fail
    BiRRTStar<Vector3f> *tree = new BiRRTStar<Vector3f>(_stateSpace);
    //  setup birrt
...
/*
TEST(birrtStarTreeTest, OutofSpacetest) {
    std::shared_ptr<GridStateSpace> _stateSpace;
    _stateSpace=make_shared<GridStateSpace>(800, 600, 40, 30);
    ...
    //  setup birrt
    tree->setStartState(Vector2f(50, 50));
    tree->setGoalState(Vector2f(900, 300));
    tree->setStepSize(30);
    tree->setMaxStepSize(30);
    tree->setGoalMaxDist(12);
    tree->setMaxIterations(5000);
